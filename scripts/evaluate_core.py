...
def test_final(data_loader, model, device):
    preds = []
    reals = []
    model.eval()
...
def statistics(preds, reals):
    ...
    correlation, p_value = stats.spearmanr(preds, reals)
...
def main(_):
    ...
    stat_loader = loader.DataLoader(dataset, batch_size=1, shuffle=False)
